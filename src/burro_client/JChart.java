...
class JChart2D extends JPanel {
    ...
    //protected Vector xData;
    
    // protected Vector yData;
    ...
    public void setData(int[] xd, int[] yd){
        // Seta os dados para fazer o Gráfico
        dataLength =xd.length;
        xData = xd;
        yData = yd;
           ...
           // }
            ...
            switch (m_chartType) {
                ...
                case LineChart:
                    ...
                    {
                        if (xData[k]!=-999) {  
                        path.lineTo(xChartToScreen(xData[k]),yChartToScreen(yData[k]));
                        } else {
                                 if ((k+1)<dataLength) path.moveTo(xChartToScreen(xData[k+1]),yChartToScreen(yData[k+1]));
                               }
                    }
                    g2.draw(path);
                    break;
